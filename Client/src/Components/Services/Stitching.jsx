...
const Stitching = () => {
    ...
    useEffect(() => {
        ...
        // Retrieve saved data and handle any errors in parsing
        ...
        if (savedBag) {
            try {
                ...
                console.error("Error parsing saved bag data:", error);
        ...
        return () => {
            localStorage.removeItem("bag_lehenga"); // Ensuring the proper item is removed
        };
    }, []);
    ...
    const handleChooseStyle = (item) => {
        const itemLink = `/${item.name.toLowerCase().replace(/\s+/g, "")}`;
        navigate(itemLink);
    };

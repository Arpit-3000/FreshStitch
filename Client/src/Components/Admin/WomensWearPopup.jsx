...
const WomensWearPopup = ({ onClose }) => {
    const [products, setProducts] = useState([]);
    const [productName, setProductName] = useState("");
    const [productPrice, setProductPrice] = useState("");
    const [editingProduct, setEditingProduct] = useState(null); // Fix: Added missing state

    // Fetch Women's Wear products from API
    useEffect(() => {
        fetch("http://localhost:4000/api/womensWear")
            .then((res) => res.json())
    ...
    const handleAddProduct = async () => {
        ...
        try {
            const response = await fetch("http://localhost:4000/api/womensWear", {
                method: "POST",
    ...
    const handleDeleteProduct = async (id) => {
        try {
            const response = await fetch(`http://localhost:4000/api/womensWear/${id}`, {
                method: "DELETE",
            });
    ...
    const handleEditProduct = async () => {
        ...
        try {
            const response = await fetch(`http://localhost:4000/api/womensWear/${editingProduct}`, {
                method: "PUT",
                headers: { "Content-Type": "application/json" },
...
export default WomensWearPopup;
